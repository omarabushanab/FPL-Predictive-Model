...
# 7 Check which nodes already have embeddings
def check_existing_embeddings(tx, node_ids):
    """Check which nodes already have embeddings"""
    query = """
    UNWIND $node_ids AS node_id
    MATCH (n) WHERE elementId(n) = node_id
    RETURN node_id, n.embedding IS NOT NULL AS has_embedding
    """
...
with conn.driver.session() as session:
    existing_embeddings = session.execute_read(check_existing_embeddings, node_ids)
...
print(f"[Step 7] {nodes_with_embeddings} nodes already have embeddings")
print(f"[Step 7] {nodes_without_embeddings} nodes need new embeddings")
...
if nodes_needing_embeddings:
    ...
    print(f"[Step 9] Created {len(embeddings)} new embeddings")
else:
    print("[Step 9] All nodes already have embeddings. Skipping creation.")
...
if nodes_needing_embeddings:
    ...
    print(f"[Step 9B] Preparing to store {total} embeddings using UNWIND...")
    ...
    print("[Step 9B] ✔ All new embeddings saved with CHUNKED UNWIND")
else:
    print("[Step 9B] No new embeddings to store")

# ---------- Create Regular Index (not vector index) ----------
def create_regular_index(tx):
    ...
    if record and record["count"] > 0:
        ...
        query = """
        CREATE INDEX embedding_property_index IF NOT EXISTS 
        FOR (n:Player) ON (n.embedding)
        """
        tx.run(query)
        return True
    return False
...
try:
    with conn.driver.session() as session:
        created = session.execute_write(create_regular_index)
        if created:
            print("[Step 10] ✔ Regular index created on embedding property")
        else:
            print("[Step 10] No nodes with embeddings found, skipping index creation")
except Exception as e:
    print(f"[Step 10] Note: Index creation might not be needed or failed: {e}")

# ---------- Custom Vector Search using Cosine Similarity ----------
def search_similar_nodes_custom(question, top_k=10):
    """Custom vector search using cosine similarity"""
    # Get query embedding
...
results = search_similar_nodes_custom(Q, top_k=10)

print("\n[Step 12] Top results from Neo4j (cosine similarity):")
...
print(f"New embeddings created in this run: {len(nodes_needing_embeddings)}")
